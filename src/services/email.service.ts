...
const transporter_TESTING = nodemailer.createTransport({
  service: "gmail",
  auth: {
    user: process.env.MAIL_USERNAME_TESTING, // Gmail address
    pass: process.env.MAIL_PASSWORD_TESTING, // App Password from Gmail
  },
});
...
export const sendMail_TESTING = async (emailContext: any) => {
  try {
    // Compile the content template first
    const contentHtml = await compileTemplate(
      emailContext.templateName,
    ...
    // Send the email
    await transporter_TESTING.sendMail(emailContext);
    console.log("Email sent successfully!");
  } catch (error) {
    console.error("Error sending email ->", error);
  }
